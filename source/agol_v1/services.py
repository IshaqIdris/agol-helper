...
##            self._count = self._getCount()
    ...
    #----------------------------------------------------------------------
    def extractDataUsingSync(self,
                  out_gdb,
                  out_fc,
                  out_attach_table="",
                  returnAttachments=True,
                  attachment_folder=None):
        """ extractDataUsingSync """
        if self.syncEnable:
            ...
            params = {'f' : 'json',
                        'layers' : string.replace(str(self._url),(os.path.dirname(self._url) + "/"),""),
            ...
            if replicaID != None:
                self.removeReplica(replicaID)

        else:
            print 'sync not enabled'
    def removeReplica(self,replicaID):
        try:

            syncURL = os.path.dirname(self._url) + "/unRegisterReplica"
            params = {'f' : 'json',
                        'replicaID' : replicaID,
                        'token' : self._token }
            syncData = urllib.urlencode(params)
    ...
    def addFeatures(self, fc, attachmentTable=None,
        ...
        try:
            ...
            print Utilities.trace()
    #----------------------------------------------------------------------
    def deleteFeatures(self, sql,objectIDs):
    ...
    #username = "<username>"
    #password = "<password>"
    #feature_service = "<feature service layer url>"
    ...
    # PASSED print fs.extractDataUsingSync(r"c:\temp\scratch.gdb", "grid", "attachments", attachment_folder=r"c:\temp\arrar")